...
const verifyJWT = async (req, res, next) => {
  try {
    ...
    if (!token) {
      ...
      if (!refreshToken) {
        return next(
          new errorResponse("Unauthorized request, please login again", 401)
        );
      }
      ...
      try {
        ...
        const user = await User.findById(decodedRefreshToken?.id).populate(
          "chats"
        );
        // console.log(user);
        ...
        if (!user || user.refreshToken !== refreshToken) {
          return next(
            new errorResponse("Invalid refresh token, please login again1", 401)
          );
        }
        ...
        return next(
          new errorResponse("Invalid refresh token, please login again2", 401)
        );
      }
    } else {
        ...
        if (error.name === "TokenExpiredError") {
          ...
          try {
            ...
            if (!user || user.refreshToken !== refreshToken) {
              return next(
                new errorResponse(
                  "Invalid refresh token, please login again3",
                  401
                )
              );
            }
            ...
            return next(
              new errorResponse(
                "Invalid refresh token, please login again4",
                401
              )
            );
          }
    ...
    return next(
      new errorResponse(error?.message || "Unauthorized request", 401)
    );
  }
};
